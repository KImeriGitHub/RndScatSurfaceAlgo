...
classdef SingleLayer_H < ops.Operators
	...
	methods(Static)
		function K = make_kernel_matrix(k, D, sigma_D, E)
			...
			%green = @(k,x,y) real(-1i/4*besselh(0,1,k*sqrt(x.^2+y.^2)));
            green = @(k,x,y) -1i/4*besselh(0,1,k*sqrt(x.^2+y.^2));
            greenNew = @(k,x,y) -1i/4*conj(besselh(0,2,conj(k)*sqrt(x.^2+y.^2)));
			if nargin == 4
                N = size(D,2);
...
% 				K = green(k,X1, X2) .* diag(sigma_D);
                for j=1:N
                    for l=1:N
                        K(j,l) = green(k,E(1,j)-D(1,l),E(2,j)-D(2,l))*sigma_D(l);
                    end
                end
			else
				...
				for i=1:N
                    ...
                    Kx(i,1:(i-1)) = sigma_D(1:(i-1)).*greenNew(k,D(1,i)-D(1,1:(i-1)),D(2,i)-D(2,1:(i-1)));
					Kx(i,(i+1):N) = sigma_D((i+1):N).*greenNew(k,D(1,i)-D(1,(i+1):N),D(2,i)-D(2,(i+1):N));
					% Diagonal terms
%                     Kx(i,i) = -1i/4*sigma_D(i)*(2*1i*(-psi(1)+log(sigma_D(i)*k/4) -1)/pi+1);
                    K(i,i) = -1i/4*sigma_D(i)*(2*1i*(-psi(1)+log(sigma_D(i)*k/2) -1)/pi+1);
					%K(i,i) = -1i/4*sigma_D(i)*(2*1i*(-psi(1) -1)/pi+1)...
                    %         *sigma_D(i)/2/pi*log(sigma_D(i)*k/4);
